# とりあえず回答をBigquery上で動かせるようにしてみた。写経は後でする。
WITH target_customer_month_log AS(
  ...
  WITH customer_month_log AS(
    ...
    CROSS JOIN work.month_mst_tb mst
  ...
  , tmp_rsvflg_log AS(
    ...
    LEFT JOIN work.reserve_tb target_rsv
      ...
      AND DATE(target_rsv.reserve_datetime)
  ...
  , rsvflg_target_log AS(
    SELECT
      *,
      ...
      -- 乱数による順位を計算
      ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY RAND())
        AS random_rank
...
, rsvflg_and_history_rsv_log AS(
  SELECT
    ...
    -- 予約日に変換
    DATE(before_rsv.reserve_datetime)
    ...
    CASE
      WHEN base.month_num =
        CAST(EXTRACT(MONTH FROM DATE(before_rsv.reserve_datetime)) AS INT)
  ...
  LEFT JOIN work.reserve_tb before_rsv
    ...
    AND before_rsv.checkin_date
        BETWEEN DATE_ADD(base.month_first_day, INTERVAL -365 DAY)
            AND DATE_ADD(base.month_first_day, INTERVAL -1 DAY)
)
...
SELECT
  ...
  COALESCE(DATE_DIFF(MAX(before_reserve_date), month_first_day, DAY), 0)
